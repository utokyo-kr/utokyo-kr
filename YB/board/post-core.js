...
import { applyNav } from "/YB/board/nav.js?v=322";
import { boardTags } from "/YB/board/board-info.js?v=322";
import { findDates } from "/YB/board/calendar.js?v=322";
  ...
  if (error || !p) {
       ...
       「사진첩 보기」 단추 하나만 깔끔하게 보입니다. */
    {
      ...
      const draw = (key, cat, title) => {
        if (!slot) return;
        slot.innerHTML = key
          ? '<a class="albumgo" href="/YB/album.html?cat=' + encodeURIComponent(cat || "daily") +
