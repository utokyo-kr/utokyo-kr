...
import { applyNav } from "/YB/board/nav.js?v=329";
import { boardInfo, tagInfo } from "/YB/board/board-info.js?v=329";
...
export async function initWrite(ORG) {
  ...
  if (!profile || !profile.approved) {
    ...
    const PUBLIC_CATS = ORG === "YB" ? ["notice", "jobs", "parttime", "exam"] : ["notice", "promo"];
    function setCat(v) {
      category = v;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        msg.textContent = "파일을 올리지 못했습니다: " + err.message +
          " — YB/board/sql/post_files.sql 을 실행하셨는지 확인해주세요.";
        return;
      }
      msg.textContent = "";
