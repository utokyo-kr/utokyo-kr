...
import { applyNav } from "/OB/board/nav.js?v=322";
import { boardInfo, boardTags, tagInfo } from "/OB/board/board-info.js?v=322";
...
async function drawAssembly(cat) {
  ...
  let mod = null;
  try { mod = await import("/OB/board/assembly-intro.js?v=322"); } catch (e) { return; }
...
export async function initBoard(ORG) {
  ...
  const CAT_YB = { notice:"공지사항", free:"자유게시판", qna:"Q&A", jobs:"구인·채용(OB/YB)", market:"벼룩시장",
                   club:"소모임", major:"전공별모임(OB/YB)", event:"행사", history:"활동 이력",
  ...
  const HOME_NAME  = "재한 도쿄대학 총동문회";
  const OTHER_NAME = "도쿄대학 한국인학생회";
  const OTHER_HOME = "/YB";
  ...
  if (!user || otherOrg) {
    ...
    ln.innerHTML = otherOrg
      ...
      '<a href="/OB/auth/login.html">로그인</a> · <a href="/OB/auth/signup.html">회원가입</a>';
    // 회원 전용 탭과 「전체」 탭 숨기기
    document.querySelectorAll("#catTabs a[data-cat]").forEach(a => {
  ...
  async function load(append = false, keepTotal = false) {
    if (memberOnlyBlocked) {   // 비로그인 상태에서 회원 전용 게시판 요청
      ...
      listEl.innerHTML = `<div class="empty"><b>${memberOnlyBlocked}</b> 게시판은 회원 전용입니다.<br><br>` +
        '<a class="btn dark" href="/OB/auth/login.html">로그인</a> ' +
        '<a class="btn line" href="/OB/auth/signup.html">회원가입</a></div>';
      moreBox.style.display = "none";
      document.getElementById("statBox").innerHTML = "";
      return;
    }
