...
//
//   · 참여마당(총회 · 소모임 · 전공별모임 · 멘토멘티 · 포럼·세미나 · 행사)과
//     경조사 · 공지 글을 봅니다. 자유·구인·장터는 보지 않습니다.
...
function monthGrid(year, month, map, todayKey, titles) {
  ...
  for (let i = 0; i < cells; i++) {
    ...
    if (ev.length) {
      html += titles
        ? ev.slice(0, 2).map((e) =>
            `<i class="chip c-${esc(e.cat)}" data-id="${esc(String(e.id))}"
                title="${esc(e.title)}">${esc(shortTitle(e.title))}</i>`).join("") +
...
export async function initCalendar(sel, opts) {
  ...
  let canWrite = false;
  try {
    const m = await import("/OB/auth/auth.js");
    const me = await m.myProfile();
    canWrite = !!(me && me.is_admin);
  } catch (e) {}
  const writeUrl = (k) => "/OB/write.html?date=" + k;
  ...
  function draw() {
    ...
    box.querySelectorAll(".cd.has").forEach((c) => {
      ...
      c.addEventListener("click", (e) => {
        ...
        if (chip) { e.stopPropagation(); location.href = "/OB/post.html?id=" + chip.dataset.id; return; }
        go();
      });
